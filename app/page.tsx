    ...
    <main style={{minHeight:'100vh',background:'#0a0a0a',color:'white',fontFamily:'sans-serif'}}>
      ...
      <section style={{textAlign:'center',padding:'80px 20px'}}>
        <h2 style={{fontSize:'48px',fontWeight:'bold',marginBottom:'16px'}}>
          Tirupati's Hidden Gems 🔥
        </h2>
        <p style={{color:'#888',fontSize:'20px',marginBottom:'40px'}}>
          Real spots. Verified by locals. Not your typical tourist guide.
        </p>
        <input 
          placeholder="Search gems... (dhabas, cafes, temples)"
          style={{width:'100%',maxWidth:'500px',padding:'16px 24px',borderRadius:'50px',border:'1px solid #333',background:'#111',color:'white',fontSize:'16px'}}
        />
      </section>
      ...
      <section style={{padding:'0 40px 60px'}}>
        ...
        <div style={{display:'flex',flexWrap:'wrap',gap:'12px',justifyContent:'center'}}>
          {['🍽️ Tiffin','🍛 Meals','🔥 Dhabas','🌮 Street Food','☕ Cafes','🏨 Homestays','🚗 Rentals','🛕 Tirumala','🏛️ Tirupati'].map(cat => (
            <button key={cat} style={{padding:'10px 20px',borderRadius:'50px',border:'1px solid #333',background:'#111',color:'white',cursor:'pointer',fontSize:'14px'}}>
              {cat}
            </button>
          ))}
        </div>
      ...
      <section style={{padding:'0 40px 80px'}}>
        ...
        <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fill,minmax(280px,1fr))',gap:'20px'}}>
          {[
            ...
            <div key={gem.name} style={{background:'#111',border:'1px solid #222',borderRadius:'16px',padding:'24px',cursor:'pointer'}}>
      ...
      {/* Footer */}
      <footer style={{textAlign:'center',padding:'30px',borderTop:'1px solid #222',color:'#444'}}>
        Built with ❤️ for Tirupati locals & travelers
      </footer>
